...
static void *boot_alloc(uint32_t n)
{
    ...
    char *result;
    unsigned long long virt_max = KERNEL_VMA + 0x100000000; // 0x10..... == 32 bit
...
void mem_init(struct boot_info *boot_info)
{
     ...
     */
    vma_list_size = ROUNDUP(MAX_VMAS * sizeof(struct vma), PAGE_SIZE);
...
static void mem_init_mp(void)
{
    ...
    for (i = 0; i < NCPU; ++i) {
        ...
        // Backed by physical mem
        boot_map_region(kern_pml4, base + KSTACK_GAP, KSTACK_SIZE,
                        PADDR(percpu_kstacks[i]), PAGE_WRITE | PAGE_NO_EXEC);
