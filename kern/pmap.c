 ...
 * Version from Wednesday, September 12th
 * ----------------------------------------------------------------------------
 * 
 * Remarks:
 * 
 * We implemented all of the features but not all of them are working properly.
 * 
 * In boot_map_region(), when trying to do the identity mapping, the program
 * crashes on random addresses (e.g. phys ffff80000fc00000 | virt fc00000)
 * and is not able to continue.
 * 
 * Consequently, we are not able to test the rest of the functions.
 * We implemented them anyway, and will discuss them on Friday, hopefully we
 * will be able to locate the errors.
 * 
 */
...
// Add static and remove it from .h file if lab2.c is not needed anymore
...
void mem_init(struct boot_info *boot_info)
{
   ...
   */
    boot_map_region(kern_pml4, USER_PAGES, 
        ...
        PADDR(pages), PAGE_WRITE | PAGE_NO_EXEC); //TODO---------------------------------
    ...
    for (vi = KSTACK_TOP-KSTACK_SIZE-KSTACK_GAP; vi < KSTACK_TOP-KSTACK_SIZE; 
         vi += PAGE_SIZE) {
        page_walk(kern_pml4, (void *)vi, CREATE_NORMAL); //-TODO--------------------------
    }
    ...
    /* Enable the NX-bit. */
    cprintf("[NX-BIT] START\n");
    // enable_wx();
    // enable_wx_elf((struct elf *)(KERNEL_VMA + (uintptr_t)boot_info->elf_hdr));

    write_msr(MSR_EFER, MSR_EFER_NXE);
    
//     __asm__ ( "#include <inc/x86-64/asm.h>;"
//               "movl $MSR_EFER, %ecx;"
//               "rdmsr;" 
//               "orl $MSR_EFER_NXE, %eax;"
//               "wrmsr;"
//     );
    ...
    cprintf("[CHECK_PAGE_INSTALLED_PML4] START\n");
    check_page_installed_pml4();
 
    /* Check if the kernel page tables fulfil W^X. */
    cprintf("[CHECK_WX] START\n");
    check_wx();

    cprintf("[CHECK_PAGE_HUGEPAGES] START\n");
    check_page_hugepages();

    cprintf("Make full identity map\n");
    ...
    physaddr_t pa, uint64_t perm)
{
    ...
    for (vi = va, pi = pa; vi < va+size; vi += PAGE_SIZE, pi += PAGE_SIZE) {
        ...
        *addr = pi | perm | PAGE_PRESENT; //-TODO-----------------------------------------
    }
}
...
static void boot_map_kernel(struct elf *elf_hdr)
{
    ...
    // boot_map_region(kern_pml4, KERNEL_VMA, 0x100000000, 0, PAGE_WRITE); //-TODO--------
    boot_map_region(kern_pml4, KERNEL_VMA, npages * PAGE_SIZE, 0, 
        PAGE_WRITE | PAGE_NO_EXEC); // | PAGE_NO_EXEC-----------TODO-------------------------------------
    kern_pml4->entries[PML4_INDEX(KERNEL_VMA)] |= PAGE_WRITE & PAGE_NO_EXEC;
    // kern_pml4->entries[PML4_INDEX(KERNEL_VMA)] |= PAGE_NO_EXEC; //-----TODO------------
...
{   
    ...
    if (!(*entry & PAGE_PRESENT)) {
        ...
        // New entry: kernel R, user R
            ...
            *entry = PADDR(new) | PAGE_PRESENT | PAGE_USER | PAGE_WRITE /*| PAGE_NO_EXEC*/; //---TODO--------
        }
    }
    return 1;
}
...
int page_insert(struct page_table *pml4, struct page_info *pp, void *va, int perm)
{
    ...
    pp->pp_ref++;
    *addr = page2pa(pp) | perm | PAGE_PRESENT; //-TODO------------------------------------
    return 0;
}
